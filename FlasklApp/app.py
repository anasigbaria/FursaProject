...
def conv(frm=1,to=1,amount=1):
    x = requests.get("http://172.28.0.1:6666/convert?cfrom="+str(frm)+"&cto="+str(to)+"&amount="+str(amount))
    return x.text


@app.route('/')
def index():
    x = requests.get("http://172.28.0.1:6666/list")
    lst= x.text
    lst= lst[17:-2].replace('[',"").replace(']',"").replace("'" , "").replace(" ","").split(",")
